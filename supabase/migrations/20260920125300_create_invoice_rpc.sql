-- Invoice + line items must be created atomically — inserting them as two separate client-side
-- calls risks an orphaned invoice with no line items if the second insert fails. SECURITY INVOKER
-- (the default) means this still runs as the calling user, so RLS on both tables still applies;
-- this function only adds atomicity, not a privilege escalation.
...
create or replace function create_invoice_with_line_items(
  ...
  p_line_items jsonb default '[]'::jsonb -- [{description, quantity, unit_price_cents, tax_rate_percent, discount_cents}]
)
returns uuid
language plpgsql
security invoker
...
begin
  ...
  loop
    ...
    insert into invoice_line_items (
      ...
      tax_rate_percent, discount_cents, sort_order
    ) values (
